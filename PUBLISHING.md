...
- 同步脚本：`<发布工具目录>/sync-release.mjs`（清空克隆目录后按规则复制，并保持工作台卡片的公开范围）
...
本地正式目录保留「轻聊」的底层代码和测试，方便后续取舍；当前正式 manifest 已移除轻聊入口，公开版同步时继续排除轻聊卡片、轻聊工具及其测试，并保持工作台卡片作为唯一功能卡。不要为了做发布副本而删除正式目录里的本地实现。
