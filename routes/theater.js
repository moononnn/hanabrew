...
function workspaceHref(surfaceSession = '', legacyToken = '') {
...
export function renderTheater(data) {
  ...
  const backHref = String(data.backHref || './visitor');
...
export default function registerTheaterRoutes(app, ctx = {}) {
  app.get('/card/theater', async (c) => {
    ...
    const backHref = workspaceHref(c.req.query('pluginSurfaceSession') || '', c.req.query('token') || '');
